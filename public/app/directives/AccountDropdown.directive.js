...
function accountDropdown( $compile, $parse ) {
	var output = {};
	...
	output.link = function( scope, element, attrs ) {
		...
		function fillDropdown( input ) {
			...
			for ( var i = 0; i < $( element ).find( "option" ).length; i++ ) {
				...
				if ( input.value != "0" && ( input.value == "" || $( element ).find( "option" )[i].value.toLowerCase().indexOf( input.value.toLowerCase() ) != -1 ) ) {
					if ( attrs.customerSupplierDropdown != undefined )
						...
						if ( value == '0' ) {
							$( '.selectize-dropdown-content' ).append( '<div class="option category" value="' + label + '" data-selectable>' + label + '</div>' );
						} else {
							$( '.selectize-dropdown-content' ).append( '<div class="option clickable" value="' + value + '" data-selectable>' + value + '</div>' );
						}
					}
				}
			}

			$( '.selectize-dropdown-content div.option.clickable' ).click( function() {
...
angular
	.module( 'bookkeeping' )
	.directive( 'accountDropdown', [ '$compile', '$parse', accountDropdown ] );
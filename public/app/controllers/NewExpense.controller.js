...
function NewExpenseCtrl( $scope, $state, $stateParams, MiscService, TRXNService ) {

	$scope.retrieve = function() {
		TRXNService.retrieve( 'expense', $stateParams.trxnId ).done( function ( response ) {
	...
	$scope.sortableStop = function() {
        $scope.reArrangeRankNumbers( $scope.transaction.expenseItems );
        $scope.reArrangeRankNumbers( $scope.transaction.expenseAccounts );
    }
	...
	$scope.initialize = function() {
		...
		MiscService.massRetrieve( [ 'account', 'account_category_type', 'account_detail_type', 'product_category', 'product_service', 'customer', 'supplier' ] ).done( function( response ) {
			...
			$scope.customerNames = $scope.setPayeeType( MiscService.extractActiveList( content.customer ) , 'Customer' );
			$scope.customerUINames = MiscService.extractUniqueNameList( content.customer );
			$scope.customerNames = $scope.customerNames.concat( $scope.setPayeeType( MiscService.extractActiveList( content.supplier ), 'Supplier' ) );
...
angular
	.module( 'bookkeeping' )
	.controller( 'NewExpenseCtrl', [ '$scope', '$state', '$stateParams', 'MiscService', 'TRXNService', NewExpenseCtrl ] );